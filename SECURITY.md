...
- The KB completion trapdoor contributes a small keyspace (exactly 2⁷, measured in 40/40 keys with working completion), and this is structural rather than a parameter choice — the grouping is forced by XOR commutativity, and an attacker with only `Rp` reconstructs it in 30/30 trials. Finding a functionally equivalent completion `R*` succeeds in 30/30 attempts by reordering the standard algorithm. See `docs/experiments/equivalent_completion_attack.md`. See README §Known limitations #2 and `docs/construction.md` §3. This is the biggest known gap — new analysis of *why* this happens or how much larger the alphabet needs to be (open problem 7) is genuinely useful; re-reporting that the keyspace is small is not.
