...
- That small table is not currently reachable by an external attacker under normal (fresh-nonce) operation — confirmed via known-plaintext testing, see `docs/experiments/known_plaintext_pair_recovery.md`. **Nonce reuse is not analyzed and is the actual open item here** — don't re-report "the table is small," a working nonce-reuse analysis (or attack) against the diffusion layer's mod-4 addition chain would be a genuinely useful report.
