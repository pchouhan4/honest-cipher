...
**Measured, not hypothetical:** at the current default parameters (`n_rules=12`, 4-generator alphabet), this failure mode is not an edge case — it's the invariable case. Across 40 fresh key generations with working completion, the substitution groups over the 16 generator-pairs are shaped `[2,2,2,2,2,2,2,1,1]` in **40/40 keys** — a keyspace of exactly 2⁷ = 128 for the pair-substitution table.

The reason is structural rather than a parameter accident. Six of the nine groups are the transposition pairs `{(a,b),(b,a)}`, in every key — and those are forced by φ being an XOR homomorphism (φ(σₐσᵦ) = φ(σᵦσₐ) by commutativity), not by anything `Rs` contributes. An attacker holding only `Rp` reconstructs the owner's exact grouping in **30/30** trials by running the same public, deterministic completion algorithm. **At these parameters, the trapdoor is not cryptographically significant** — and §2 of `docs/experiments/equivalent_completion_attack.md` shows why scaling is unlikely to change that. Confidentiality in the current implementation rests on the HMAC-SHA256 keystreams in `encode()`/`diffuse()`, not on the rewriting system.

> **Correction (supersedes an earlier version of this paragraph).** A previous revision reported "median |Rs \ Rp| ≈ 8 rules, 12.5% of trials added zero new rules." Those figures were computed without checking `knuth_bendix_complete()`'s `success` flag, at a time when a bug in that function's success criterion caused it to report failure on 100% of runs and `generate_kb_key()` to silently fall back to the trivial `Rs = Rp` on every key generation. The old numbers therefore described prematurely-terminated rule sets, not genuine completions. The bug is fixed (see `docs/experiments/equivalent_completion_attack.md` §0 and the regression tests in `tests/test_honest.py::TestKBCompletion`); the numbers above are re-measured against working completion. The conclusion survived re-measurement, but the earlier premise was wrong and is corrected here rather than quietly restated.
