...
- Rs-equivalence groups over the 16 generator-pairs are almost always shaped `[2,2,2,2,2,2,2,1,1]` — seven pairs-of-two and two fixed points. A substitution table built from that shape has a keyspace of **roughly 2⁷ = 128 possibilities** — brute-forceable by hand, let alone by computer.
- In **12.5% of key generations (5/40 trials), KB completion added zero new rules** — `Rs == Rp` exactly, meaning the grouping was fully derivable from the *public* rules alone for that key. In that case `kb` mode silently degrades to `block` mode with no different security property, and nothing in the output tells you this happened.
- Even when new rules are added (median ~8 per key), essentially all of the original `Rp` rules survive into `Rs` unchanged — so `Rs \ Rp`, the thing this document calls "the private information" below, is usually 0-1 rules' worth of difference on the wire.

**What this means concretely: whatever confidentiality this cipher has right now comes almost entirely from the HMAC-SHA256 keystreams in `encode()` and `diffuse()`, not from the rewriting-system trapdoor.** The trapdoor is real code, doing real Knuth-Bendix completion — it is not decorative in the sense of being fake — but at current parameters it contributes single-digit bits of keyspace, sometimes zero, to a construction whose entire point is to explore whether a rewriting-system trapdoor *can* carry security. Scaling the alphabet/rule length (open problem 7) is a prerequisite for that question to even be tested, not a nice-to-have.
